...
2. 「AIレビュー実行 (N)」を押すと、PR ごとに tmux セッション `gh-review` 内で対話モードの `claude` が起動します（ペインは tiled レイアウトで並び、ペイン枠に PR 名が表示されます）。未アタッチなら Terminal.app が自動で開きます。
3. 起動済みの PR は AI 列に「tmux」ボタンが出ます。既にアタッチ済みのレビューウィンドウがあればそれを前面に表示し、なければ新しくアタッチしたターミナルを開きます。
