...
6. レビューエージェントが `$HOME/.claude/pr-review-logs/<repo>/<PR番号>-<日時>.html` に出力した HTML レポートは、**report 列**にタイムスタンプ（`MM/DD hh:mm`）のボタンとして表示されます。クリックでデフォルトブラウザの**専用ウィンドウ**にタブとして開きます（herdr のレビュータブと同じ発想で、レポートは1つのウィンドウに集約されます。Safari / Chrome 系ブラウザで対応。Arc など対応していないブラウザでは通常の `open` にフォールバックします）。同じ PR に複数レポートがある場合は新しい順にすべて並びます。一覧は「⟳ 更新」のたびに再スキャンされます。
