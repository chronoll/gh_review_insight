...
- **status**: オープン PR を一覧表示。PR の id・タイトルをクリックすると GitHub ページがブラウザで開きます。
  - 各 PR を3つの状態に分類し、**行全体を色分け**します:
    - 要レビュー（赤系）= リクエストがあるが誰もレビューしていない
    - 要レビュー(他者済)（橙系）= リクエストがあり自分は未レビュー、誰かがレビュー済み
    - レビュー済（緑系）= 自分がレビュー済み
...
- status: フィルタ（title / repo / author）で絞り込み。PR 行の id・タイトルがリンクになっており、クリックで GitHub へ遷移。ホバーするとレビュー履歴などの詳細が出ます。行の背景色は状態（要レビュー / 他者済 / レビュー済）を表します。
- stats: `days` を変更して `更新` を押すと集計期間が変わります。
- 設定: `⚙ 設定` でユーザーごとの文字色を割り当て（color picker）。設定は `$HOME/.config/gh-review-insight/colors.json` に保存され、次回起動時に読み込まれます。
