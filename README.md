...
status ビューで PR を選択し、まとめて Claude Code にレビューさせられます。実行は**対話モード**なので、進行の様子を通常利用と同じ画面で確認でき、完了後はそのままタブ内で会話を継続できます。ホストは [herdr](https://herdr.dev) がインストールされていれば herdr（推奨）、なければ tmux です。
...
2. 「AIレビュー実行 (N)」を押すと、PR ごとに対話モードの `claude` がタブとして起動します（タブ名は `repo#番号`）。
   - **herdr**: 専用 workspace `gh-review` に1 PR = 1タブで起動します。herdr クライアントが未アタッチならターミナル（Ghostty 優先、なければ Terminal.app）で `herdr` を自動で開きます。
   - **tmux**（herdr 未インストール時）: セッション `gh-review` に1 PR = 1ウィンドウで起動します。タブ切り替えはステータスバーのタブ名クリック（`mouse on` 設定済み）または `Ctrl-b n`/`p`/`w`。Shift+Enter 改行のための extended-keys も設定されます。
3. 起動済みの PR は AI 列に「herdr」/「tmux」ボタンが出ます。押すと**その PR のタブに切り替えたうえで**、レビューウィンドウを前面に表示します（なければ新しくターミナルを開きます）。Ghostty はウィンドウ単位の操作に対応していないため、前面表示はアプリごと行います。起動済み状態は `ai_launched.json` に永続化され、タブが生きている限りアプリを再起動してもボタンが復元されます（タブを閉じた分は起動時に自動で片付きます）。
4. 起動に失敗した場合は AI 列に「失敗」と表示されます（ホバーでエラー内容を確認し、再選択して実行し直せます）。

仕組みと注意:

- herdr または tmux が必要です（`brew install herdr` / `brew install tmux`）。
...
- レビュータブはアプリを終了しても残ります（herdr サーバー / tmux セッションが保持）。claude を exit すればタブは閉じます。
