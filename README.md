...
1. 行頭のチェックボックス（「選択」列）で任意の PR を選択します。ツールバーの「未対応を選択」で `waiting` / `should` の一括選択、「解除」で全解除もできます。実行中のレビュータブがある PR は二重起動防止のため選択できません（タブを閉じれば再び選択できます）。ただし**再リクエストされた PR は例外**で、タブが実行中でも選択できます（次項参照）。
2. 「AIレビュー実行 (N)」を押すと、PR ごとに対話モードの `claude` が herdr のタブとして起動します。herdr クライアントが未アタッチならターミナル（Ghostty 優先、なければ Terminal.app）で `herdr` を自動で開きます。
   - **再リクエスト時の差分レビュー**: 一度 AI レビューした PR が、対応後に再度レビュー依頼された（status が再び `waiting` に戻った）場合は、新規セッションではなく**前回のレビューのセッションを `claude --resume` で継続**し、「再リクエストされたので前回のレビュー以降の差分に注目してレビューして」という指示を自動で送ります。claude 自身が前回指摘した内容を覚えたまま差分を見られるので、フル差分を読み直すより焦点が絞れます。対象の PR はチェックボックスに再リクエストである旨がホバー表示されます。
3. 起動済みの PR は AI 列に「herdr」ボタンが出ます。押すと**その PR のタブに切り替えたうえで**、レビューウィンドウを前面に表示します（なければ新しくターミナルを開きます）。Ghostty はウィンドウ単位の操作に対応していないため、前面表示はアプリごと行います。起動済み状態は `ai_launched.json` に永続化され、タブが生きている限りアプリを再起動してもボタンが復元されます。
4. PC 再起動などで herdr サーバーごとタブが失われた場合は、AI 列に「続き」ボタンが出ます。claude の session id は起動時に `--session-id` で割り当てて保存してあるので、押すと `claude --resume` で**同じ会話を新しいタブに再現**できます（レビューの transcript 自体は `~/.claude/projects/` に残っているため、タブが消えても中身は失われません）。この機能追加より前に起動した分（session id を保存していない）は復元できず、起動時に一覧から消えます。
5. 起動に失敗した場合は AI 列に「失敗」と表示されます（ホバーでエラー内容を確認し、再選択して実行し直せます）。
6. レビューエージェントが `$HOME/.claude/pr-review-logs/<repo>/<PR番号>-<日時>.html` に出力した HTML レポートは、**report 列**にタイムスタンプ（`MM/DD hh:mm`）のボタンとして表示されます。クリックで**常に Chrome の専用ウィンドウ**にタブとして開きます（OS のデフォルトブラウザ設定とは独立。herdr のレビュータブと同じ発想で、レポートは1つのウィンドウに集約されます）。Chrome がインストールされていない場合のみ OS のデフォルトブラウザにフォールバックします（Safari 系は同じ専用ウィンドウ集約に対応、それ以外は通常の `open`）。同じ PR に複数レポートがある場合は新しい順にすべて並びます。一覧は「⟳ 更新」のたびに再スキャンされます。
...
- タブが多数（数十件）同時に開いていると、herdr CLI の応答が一時的に不安定になり、実際には起動に成功しているのに「失敗」と誤表示されることがあります（herdr 呼び出しは自動で数回リトライしますが、それでも解消しない場合があります）。誤って「失敗」表示になった行は、`ai_launched.json` に該当 PR の `pane_id`（herdr の `pane list` の `terminal_id`）・`session_id`（実行中の `claude` プロセスの `--session-id` 引数から確認可能）・`window_name` を手動で追記し、アプリを再起動すると復元できます。
