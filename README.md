...
status ビューで PR を選択し、まとめて Claude Code にレビューさせられます。実行は tmux セッション内の**対話モード**なので、進行の様子を通常利用と同じ画面で確認でき、完了後はそのまま各ペインで会話を継続できます。

1. 行頭のチェックボックス（「選択」列）で任意の PR を選択します。ツールバーの「未対応を選択」で `waiting` / `should` の一括選択、「解除」で全解除もできます。実行中の tmux タブがある PR は二重起動防止のため選択できません（タブを閉じれば再び選択できます）。
2. 「AIレビュー実行 (N)」を押すと、PR ごとに tmux セッション `gh-review` 内で対話モードの `claude` が起動します（PR ごとに1つの tmux ウィンドウ＝画面下部のタブになり、タブ名に `repo#番号` が表示されます）。未アタッチならターミナルが自動で開きます（Ghostty がインストールされていれば Ghostty、なければ Terminal.app）。
3. 起動済みの PR は AI 列に「tmux」ボタンが出ます。押すと**その PR のタブに切り替えたうえで**、アタッチ済みのレビューウィンドウを前面に表示します（なければ新しくアタッチしたターミナルを開きます）。Ghostty はウィンドウ単位の操作に対応していないため、前面表示はアプリごと行います。起動済み状態は `ai_launched.json` に永続化され、tmux 側のタブが生きている限りアプリを再起動しても「tmux」ボタンが復元されます（タブを閉じた分は起動時に自動で片付きます）。
4. タブの切り替えは、画面最下部のステータスバーのタブ名を**マウスでクリック**（`mouse on` 設定済み）、またはキーボード（`Ctrl-b n`/`p` で隣、`Ctrl-b w` で一覧選択）で行えます。tmux がマウスを掴むため、ターミナルのネイティブなテキスト選択には Shift を併用してください（tmux 上のドラッグ選択はそのまま tmux のコピーになります）。
4. 起動に失敗した場合は AI 列に「失敗」と表示されます（ホバーでエラー内容を確認し、再選択して実行し直せます）。

仕組みと注意:

- tmux が必要です（`brew install tmux`）。
...
- tmux セッションはアプリを終了しても残ります。不要になったら `tmux kill-session -t gh-review` で片付けられます。
