...
2. 「AIレビュー実行 (N)」を押すと、PR ごとに tmux セッション `gh-review` 内で対話モードの `claude` が起動します（ペインは tiled レイアウトで並び、ペイン枠に PR 名が表示されます）。未アタッチならターミナルが自動で開きます（Ghostty がインストールされていれば Ghostty、なければ Terminal.app）。
