...
6. レビューエージェントが `$HOME/.claude/pr-review-logs/<repo>/<PR番号>-<日時>.html` に出力した HTML レポートは、**report 列**にタイムスタンプ（`MM/DD hh:mm`）のボタンとして表示されます。クリックでデフォルトブラウザの**新規ウィンドウ**で開きます（Safari / Chrome 系ブラウザ。それ以外のブラウザではタブとして開く場合があります）。同じ PR に複数レポートがある場合は新しい順にすべて並びます。一覧は「⟳ 更新」のたびに再スキャンされます。
